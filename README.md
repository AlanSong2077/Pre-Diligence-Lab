...
**Pre-Diligence Lab** 是面向一级市场投研人员的财务风控调研系统，主要用于支尽调、风控等场景下进行数据调研、财务指标计算、财务风控分析。

本应用将分散在 SEC EDGAR、HKEXnews、巨潮资讯、东方财富、中基协 AMAC、Yahoo Finance 等十余个公开数据源的财务披露、行情、新闻、公告、备案信息聚合到统一界面，所有用户输入（搜索历史、收藏、API Key、配置）均存储在本地，**不向任何第三方服务端上传**。
为保证数据来源可信，LLM 仅为可选功能。数据均从公开API端口获得，所以未配置时所有数据抓取、计算、图表功能亦可正常使用，仅无法利用Agent能力进行深度分析。
如果喜欢这个项目，欢迎大家点击 Star 和 Follow ！后续版本迭代将第一时间通知，感谢大家！

---

## 下载与安装

### 方式一：普通用户推荐使用预编译好的文件直接安装
...
该功能暂未开启。需要提供天眼查/企查查API Key通过接口统一访问数据。数据包括：统一社会信用代码 / 公司名 / 股票代码检索，展示工商注册基本信息，并提供以下四类公开层穿透，股权穿透、投资信息最多允许穿刺到三级。
...
作为补充能力，引入LLM，支持对查询内容进行问答讲解，上下文在会话内保留，关闭后上下文不再维护。
...
| [巨潮资讯](https://www.cninfo.com.cn) | A 股定期报告、临时公告、招股书 |
| [东方财富](https://data.eastmoney.com) | A / H / US 行情、财务数据 |
| [中基协 AMAC](https://www.amac.org.cn) | 私募基金管理人、基金备案 |
| [Yahoo Finance](https://query1.finance.yahoo.com) | 美 / 港 / 全球行情 |
...
**Q4. 海外数据无法查询**
更换IP、WI-FI、VPN，存在端口封禁，或者SSL非合法签名。请使用1.0.2以上版本，优化了此问题。
...
本项目仅为投研财务风控辅助工具开源，旨在帮助小型机构研究人员简化数据流，降低数据收集成本。**使用者应自行核实所有数据，并对投资决策负全部责任**。
