...
本应用将分散在 SEC EDGAR、HKEXnews、巨潮资讯、东方财富、中基协 AMAC、Yahoo Finance 等十余个公开数据源的财务披露、行情、新闻、公告、备案信息聚合到统一界面，所有用户输入（搜索历史、收藏、API Key、配置）均落盘在本地 `~/.prediligencethelab/`，**不向任何第三方服务端上传**。LLM 为可选功能，未配置时所有数据抓取、计算、图表功能亦可正常使用，仅无法利用Agent能力进行深度分析。
